...
def main():
    ...
    if len(args) != 1:
        parser.error('Must provide a pathway to metadata table (txt file) that includes four columns [prefix of output file] [output-directory] [BAM file pathway]  and [BED file pathway].')
    else:
    ...
    for i, row in metadata.iterrows():
        ...
        reads_counter = crpb.CountReadsPerBin([bam], bedFile=bed, numberOfProcessors=options.nproc)
